...
CHECKPOINT_PATH = os.path.join(
    ...
    "./ray_results/PPO_sp/checkpoint_002500/checkpoint-2500",
)
POLICY_NAME = "default"


def _policy_mapping_fn(agent_id, *args, **kwargs):
    return POLICY_NAME


class _CompatUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        if name == "_policy_mapping_fn":
    ...
    def __init__(self, env: gym.Env):
        ...
        if os.path.exists(config_path):
            with open(config_path, "rb") as f:
                config = _CompatUnpickler(f).load()
        else:
